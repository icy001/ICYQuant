"""Tests for the order factory (Commit 33 Part 1.1 / 1.2)."""
...
def test_create_order_from_handoff_request(
    factory: OrderFactory, make_handoff_request, make_create_command
):
...
def test_order_preserves_authorization_lineage(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    request = make_handoff_request(
        intent_id="INT-001",
    ...
    order = factory.create(request, make_create_command(request))
...
def test_order_requires_handoff_request(
    factory: OrderFactory, make_request, make_create_command
):
    request = make_request(state=OrderRequestState.NORMALIZED)

    with pytest.raises(OrderCreationError):
        factory.create(request, make_create_command(request))
...
def test_no_order_from_pre_handoff_states(
    factory: OrderFactory, make_request, make_create_command, state
):
    with pytest.raises(OrderCreationError):
        factory.create(
            make_request(state=state),
            make_create_command(make_request(state=state)),
        )
...
def test_factory_converts_to_exact_decimal(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    request = make_handoff_request(
        order_type="LIMIT",
...
def test_factory_decimal_conversion_is_not_binary_float(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    request = make_handoff_request(
        order_type="LIMIT",
...
def test_command_must_target_the_same_request(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    request = make_handoff_request()
    ...
    with pytest.raises(OrderCreationError):
        factory.create(request, make_create_command(other))


def test_factory_rejects_negative_quantity(
    factory: OrderFactory, make_handoff_request, make_create_command
):
...
def test_factory_rejects_none_request(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    command = make_create_command(make_handoff_request())
    with pytest.raises(OrderCreationError):
        factory.create(None, command)  # type: ignore[arg-type]


def test_order_side_is_normalized_from_request(
    factory: OrderFactory, make_handoff_request, make_create_command
):
...
def test_order_metadata_is_copied_from_request(
    factory: OrderFactory, make_handoff_request, make_create_command
):
...
def test_created_at_and_updated_at(
    ...
    command = make_create_command(
        request, timestamp=datetime(2026, 8, 13, 9, 30, 0)
    )
...
def test_created_order_has_no_reject_reason(
    factory: OrderFactory, make_handoff_request, make_create_command
):
    order = factory.create(
        make_handoff_request(), make_create_command(make_handoff_request())
    )
