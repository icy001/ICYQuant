"""
ICYQuant AI Research Health — health check and circuit breaker for the research platform.

Monitors the health of all research platform components and provides
readiness/liveness probes for orchestration systems.
"""
...
class ResearchHealthChecker:
    """Health check and circuit breaker for the research platform.
    ...
    def check_all(self) -> dict[str, str]:
        """Check all components and return status map."""
        import time
        now = time.time()
    ...
    def is_ready(self) -> bool:
        """Readiness probe: platform is ready to accept requests."""
        return self._overall_status in (HealthStatus.HEALTHY, HealthStatus.DEGRADED)

    def is_alive(self) -> bool:
        """Liveness probe: platform process is alive."""
    ...
    def get_readiness_response(self) -> dict[str, Any]:
        ...
        return {
            ...
            "components": {
                name: c.status.value for name, c in self._components.items()
            },
        }
